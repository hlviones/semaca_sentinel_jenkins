pipeline {
    ...
    stages {
        ...
        stage('semaca_sentinal/semaca_sentinal_Smoke_Test - Checkout') {
            steps {
                checkout([
                    $class: 'GitSCM',
                    branches: [[name: '**']],
                    ...
                    extensions: [],
                    submoduleCfg: [],
                    userRemoteConfigs: [[credentialsId: 'REDACTED', url: 'git@github.com:hlviones/semaca-sentinal.git']]
                ])
            }
        }
        ...
        stage('Runs the job and waits for completion') {
            steps {
                sh '''
                    ...
                    bash ./$SCRIPT
                    set +x
                    while ! grep "COMPLETE" $LOG_FILE; do
        ...
        post {
            success {
                script {
                    echo "Adding tag PASSED_SMOKE_${env.BUILD_ID}"
                    sh "git tag PASSED_SMOKE_${env.BUILD_ID}"
                    sh 'git push --tags'
                }
            }
        }
